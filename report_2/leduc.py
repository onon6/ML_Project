...
flags.DEFINE_integer("num_train_episodes", int(20e6), "Number of training episodes.")
...
def train_network(
    ...
    logging.info(
        "Training network with hyperparameters: LAY_SIZE={}, REPBUFCAP={}, RESBUFCAP={}, ANTPARAM={}, ESTART={}".format(
            hidden_layers_sizes,
            replay_buffer_capacity,
            reservoir_buffer_capacity,
            anticipatory_param,
    ...
    with tf.Session() as sess:
        agents = [
            nfsp.NFSP(
                sess, idx, info_state_size, num_actions, hidden_layers_sizes, **kwargs
            )
        ...
        for ep in range(num_episodes):
            ...
            if (ep + 1) % FLAGS.eval_every == 0:
                ...
                logging.info(
                    "[%s/%s] AVG Exploitability %s", ep + 1, num_episodes, expl
                )
...
def random_search():
    # Perform a random search through the search space

    # Set the valid parameter values
    ...
    len_perms = (
        len(HLAY_SIZES) * len(REPBUFCAP) * len(RESBUFCAP) * len(ANTPARAM) * len(ESTART)
    )
    permutations = list(itertools.product(*allparams))
    assert len(permutations) == len_perms

    pipeline_idx = floor(len_perms / 2)
    danilo_range = range(1, pipeline_idx)
    niels_range = range(pipeline_idx, len_perms)

    #### Set the Random Search parameters
    num_param_combs = 20  # number of parameter combinations to consider
    range_idx = niels_range  # value range - danilo_range or niels_range
    num_episodes = int(1e6)  # episodes to train for
    ####

    perm_idxs = random.sample(range_idx, num_param_combs)
    network_summaries = dict()
    ctr = 1

    # Run for every parameter combination
    for perm_idx in perm_idxs:
        logging.info("==================================================")
        logging.info("Starting episode {}/{}".format(ctr, num_param_combs))
        logging.info("==================================================")
        hp = permutations[perm_idx]
        episodes, exploits, nashes = train_network(
            num_episodes, hp[0], hp[1], hp[2], hp[3], hp[4]
        )
        network_summaries[perm_idx] = exploits[-1]

        # Store the calculations in a temporary file
        filename = "./checkpoints/" + str(perm_idx) + ".npy"
        np.save(filename, exploits[-1])
        ctr += 1

    network_summaries["all_perms"] = permutations
    np.save("leduc_network_summary.npy", network_summaries)


def find_best_network():
    # Find the best network from a range of permutations on available data
    ...
    # Select the directory with checkpoints
    directory = os.fsencode("./checkpoints")

    # Loop all files - parameter combinations - and select the one with the best exploitability
    min_expl = float("inf")
    min_file = None
    ...
    logging.info(
        "Filename {} contained lowest exploitability of {}".format(
            min_file, str(min_expl)
        )
    )
    splitted = min_file.split(".")
    idx = splitted[0]

    # Get the parameters from the best run and return their values
    hp_optim = permutations[int(idx)]
    return hp_optim


def save_best_network():
    # Save the best network, found via Random Search, in a separate file
    hp = find_best_network()
    episodes, exploits, nashes = train_network(
        int(20e6), hp[0], hp[1], hp[2], hp[3], hp[4]
    )
    d = dict()
    d["episodes"] = episodes
    d["exploits"] = exploits
    d["nashes"] = nashes
    np.save("leduc_best.npy", d)


def plot_best_network():
    # Plot the exploitability for the best network
    d = np.load("./leduc_best.npy", allow_pickle=True).item()
    episodes = d["episodes"]
    exploits = d["exploits"]
    nashes = d["nashes"]
    print(exploits[-1])
...
def __tabular_policy_from_csv(game, filename):
    # Export the tabular policy, trained for the game, to a csv file
